...
def performPCA(data, k=4096):
    # preprocess the data
    data_mean = torch.mean(data, -1, True)
    data_X = data - data_mean.expand_as(data)
    # svd
    U, S, V = torch.svd(torch.t(data_X))
    pca = torch.mm(data_X, U[:, :k])
...
class NetVLAD(nn.Module):
    ...
    def __init__(self, num_clusters=64, dim=128, 
                 normalize_input=True, vladv2=False, reduction=False):
        """
        Args:
            num_clusters : int
    ...
    def forward(self, x):
        ...
        # print('before reduction = ', vlad.shape)
        # if self.reduction:
        #     vlad = performPCA(vlad)
        #     print('after reduction = ', vlad.shape)
